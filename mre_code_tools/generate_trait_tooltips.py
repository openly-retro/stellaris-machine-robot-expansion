...
def create_tooltip_for_leader(
    ...
    will_use_custom_tt = False
    # Lower the custom tooltip because Shroud knows whether it's gonna be capitalized or not
    if potential_custom_tooltip := root.get('custom_tooltip', '').lower():
    ...
    if not will_use_custom_tt:
        ...
        for modifier_key in TRAIT_MODIFIER_KEYS:
            if root.get(modifier_key):
                ...
                # Remove any garbage modifiers from the trait's modifiers, including 'mult'
