	...
	on_success = {{
		...
		from = {{
			oxr_mdlc_country_create_mover_fleet_give_instructions = {{
				...
				FLAG = {mining_or_research_planet_flag}
			}}
		}}
	}}
...
def make_orbital_extract_special_projects_file_data():
	...
	for game_resource in GAME_RESOURCES_NAMES:
		# Determine which planet flag to send the ship to
		orbital_deposit_type = ORBITAL_DEPOSIT_SHIPCLASS_TYPES[game_resource]

		planet_flag = ORBITAL_TARGET_PLANET_FLAGS['mining']
		if orbital_deposit_type == RESEARCH_STATION:
			planet_flag = ORBITAL_TARGET_PLANET_FLAGS['research']
		...
		special_project_for_resource = special_project_template.format(
			...
			mining_or_research_planet_flag=planet_flag
		)
